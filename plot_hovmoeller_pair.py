...
# Load datasets
ds = [xr.open_dataset(f"./data_timeseries_hovmoeller/{f}") for f in files]
...
def plot_hovmoller_pair(var, dataset1, dataset2):
    ...
    # Remove commas and decimals from colorbar labels
...
for var in rssh_aviso:
    if var in rssh_oras5:
        plot_hovmoller_pair(var, ds[0], ds[1])
